...
function sendPopulateFriendShip(friendShip: IFriend, me: string, responseOptions: {response: any, status: number}) {
    friendShip.populate(FriendShipPopulateOptions, (err, populateFriend) => {
    ...
    Friend.findOne()
        ...
        .then(friendship => {
            ...
            friendship
                .save()
                .then(
                    _friendShip => sendPopulateFriendShip(_friendShip, decodedToken._id, { response: res, status: 200 }),
                    err => res.status(500).json({ code: err.code || 0, description: err.message })
                )
        }, err => res.status(500).json({ code: err.code || 0, description: err.message }))
}
...
export function removeFriend(req, res) {
    ...
    Friend
        .deleteOne({ $or: [{ from: { $eq: id }, to: { $eq: friendID } }, { from: { $eq: friendID }, to: { $eq: id } }] })
        .where("state")
        .ne(FriendShip.State.PENDING)
        .then(result => {
            ...
            removeChatBetween(friendID, decodedToken._id)
            return res.status(200).json({ description: "Friendship is over." })
        }, err => res.status(500).json({ code: err.code || 0, description: err.message }))
}

function removeChatBetween(friend: string, me: string) {
    Chat.findOne({ "users.user": { $all: [Types.ObjectId(friend), Types.ObjectId(me)] } })
        ...
        .then(chat => {
            ...
            chat.save()
                .then(data => console.debug("remove chat ", data._id), err => console.error("FriendShip: remove chat : err = ", err))
