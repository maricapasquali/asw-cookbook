...
export function addFood(req, res) {
    ...
    function sendNewListPoint(_doc: IShoppingList) {
        console.debug(_doc)
        _doc.populate("shoppingList.food", (err, docPopulate) => {
            if (err) return res.status(500).json({ description: err.message })
            return res.status(201).json(docPopulate.shoppingList.pop())
        })
    }

    function handlerError(_err: any, options: {newList?:boolean, newPoint?: boolean}) {
        console.error(_err)
        if (MongooseValidationError.is(_err)) return res.status(400)
            .json({ description: _err.message })
        if (MongooseDuplicateError.is(_err)) {
            return res.status(409).json({
                description: options.newList ? "User already owns a list." : options.newPoint ? "Food has been already inserted in the shopping list." : "Duplicate."
            })
        }
        ...
        .then(doc => {
            if (doc) {
                ...
                new ShoppingList({ user: user._id, shoppingList: [body] })
                    .save()
                    .then(_doc => sendNewListPoint(_doc), err => handlerError(err, { newList: true }))
            }
...
export function list(req, res) {
    ...
    ShoppingList
        ...
        .then(
            doc => res.status(200).json(doc ? doc.shoppingList : []),
            err => res.status(500).json({ code: err.code || 0, description: err.message })
        )
}
...
function _updateFoodOnShoppingList(req: any, res: any, options: { body?: { checked: boolean }, responseDescription: string }) {
    ...
    ShoppingList
        ...
        .then(doc => {
            ...
            const index = doc.shoppingList.findIndex(p => p._id == pointShoppingListID)
            ...
            doc.save()
                .then(
                    () => res.status(200).json({ description: options.responseDescription }),
                    err => res.status(500).json({ code: err.code || 0, description: err.message })
                )
        }, err => res.status(500).json({ code: err.code || 0, description: err.message }))
}

export function updateFoodOnList(req, res) {
    _updateFoodOnShoppingList(req, res, { responseDescription: "Shopping List point has been updated", body: req.body })
}

export function deleteFoodOnList(req, res) {
