...
function addCommentOn(doc: IComment | IRecipe, options: {body: any, userId: string, recipe?: string}, res: any) {
    const { body, userId, recipe } = options
        ...
        .then(
            _doc => {
                ...
                doc
                    .save()
                    .then(() => {
                        _doc.populate({ path: "user", select: PopulationSelectUserComment }, (err, populateComment) => {
...
function commentToRecipe(req: any, res: any) {
    ...
    Recipe
        ...
        .then(recipe => {
            ...
            return addCommentOn(recipe, { body: req.body, userId: user?._id }, res)
        }, err => res.status(500).json({ code: err.code || 0, description: err.message }))
}
...
function responseToComment(req: any, res: any) {
    ...
    addCommentOn(comment, { body: req.body, userId: user?._id, recipe: req.params.recipeID }, res)
}
...
function report(req, res) {
    ...
    comment.reported.push(new Report({ user }))
    comment
        .save()
        .then(
            () => res.status(200).json({ description: "Comment has been reported." }),
            err => res.status(500).json({ code: err.code || 0, description: err.message })
        )
}
...
function unReport(req, res) {
    ...
    comment
        .save()
        .then(
            () => res.status(200).json({ description: "Comment has been unreported." }),
            err => res.status(500).json({ code: err.code || 0, description: err.message })
        )
}
...
function changeContentComment(req, res) {
    ...
    comment
        .save()
        .then(
            () => res.status(200).json({ description: "Comment's content has been changed." }),
            err => res.status(500).json({ code: err.code || 0, description: err.message })
        )
}
        ...
        .then(_comment => {
            ...
            if (accessManager.isAdminUser(user) && _comment.user) {
                ...
                User.updateOne({ _id: _comment.user._id }, { $inc: { strike: 1 } })
                    .then(
                        result => console.debug(result.n > 0 && result.nModified > 0 ? "Add strike." : "No ad strike"),
...
export function listReportedComments(req, res) {
    Comment
        .find()
        .where("reported")
        .ne([])
