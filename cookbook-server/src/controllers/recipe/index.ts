...
import {
    IRecipe,
    RecipePopulationPipeline
} from "../../models/schemas/recipe"
import {
    RecipeType,
    UpdateAction
...
function sendPopulatedRecipe(recipe: IRecipe, res: any, status: number): void {
    recipe.populate(RecipePopulationPipeline, (err, recipe) => {
...
export function createRecipe(req, res) {
    new Recipe(req.body)
        .save()
        .then(
            recipe => sendPopulatedRecipe(recipe, res, 201),
            err => {
...
function queryListRecipes(user: string, type: string, filters: any): Query<IRecipe[], IRecipe, object, IRecipe> {
...
export function listRecipes(req, res) {
    ...
    function timestampMyLike(recipe: IRecipe): number {
        return recipe.likes.find(l => l.user?._id == id)?.timestamp
    }
...
export function oneSharedRecipe(req, res) {
...
export function oneRecipe(req, res) {
    const { id, recipeID } = req.params
    const { type } = req.query
    switch (type as RecipeType) {
        case RecipeType.SHARED:
            oneSharedRecipe(req, res)
            break
        default: {
            let query: Query<IRecipe, IRecipe, object, IRecipe> = null
            switch (type) {
                case RecipeType.SAVED:
...
function updateActualRecipe(req, res) {
    const { id, recipeID } = req.params
    const body = req.body
    ...
    Recipe
        ...
        .then(recipe => {
            ...
            const updatedRecipe = new Recipe(Object.assign(recipe, body))
            updatedRecipe.save()
                .then(
                    _doc => sendPopulatedRecipe(_doc, res, 200),
                    err => {
                        if (MongooseValidationError.is(err)) return res.status(400).json({ description: err.message })
...
function updatePermissionRecipe(req, res) {
    const { recipeID } = req.params
    ...
    Recipe
        ...
        .then(recipe => {
            ...
            _revoke.forEach(p => {
                const index = recipe.permission.findIndex(pp => pp.user._id == p.user)
                if (index !== -1) recipe.permission.splice(index, 1)
                else _unRevoked.push(index)
            })

            _update
                .map(p => ({ user: p.user, granted: p.granted || IPermission.GrantedType.READ }))
                .forEach(p => {
            ...
            recipe.save()
                .then(_doc => {
                    _doc.populate(RecipePopulationPipeline, (err, populateRecipe) => {
                        if (err) return res.status(500).json({ description: err.message })
...
export function updateRecipe(req, res) {
    ...
    switch (field as UpdateAction) {
        case UpdateAction.PERMISSION:
            return updatePermissionRecipe(req, res)
        default:
            return updateActualRecipe(req, res)
    }
}
    ...
    Recipe
        ...
        .then(recipe => {
            ...
            recipe
                .remove()
                .then(
                    doc => res.status(200).json({ description: "Recipe has been deleted", deletedDoc: doc }),
                    err => res.status(500).json({ code: err.code || 0, description: err.message })
                )
...
export function listAllRecipes(req, res) {
