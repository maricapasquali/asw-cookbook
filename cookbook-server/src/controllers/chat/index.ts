...
export function createChat(req, res) {
    ...
    function _createChat() {
        Chat
            .find({ "users.user": { $all: body.users.map(u => u.user) }, "info.name": body.info.name })
            .then(chats => {
                ...
                new Chat(body)
                    .save()
                    .then(chat => {
                        chat.populate(ChatPopulationPipeline, (err, populateChat) => {
    ...
    if (body.users.some(i => i.user == RBAC.Role.ADMIN)) {
        ...
        User.find()
            ...
            .equals(RBAC.Role.ADMIN)
            .then(admins => {
                if (admins.length === 0) return res.status(404).json({ description: "Admins are not found" })
                const index = body.users.findIndex(i => (i.user == RBAC.Role.ADMIN))
                const adminRole = body.users[index].role
...
export function listChat(req, res) {
    const { id } = req.params
    ...
    Promise.all([
        Chat.find(filtersGroup.filter).populate(filtersGroup.pipeline),
        Chat.find(filtersOne.filter).populate(filtersOne.pipeline)
    ])
        .then(results => {
            ...
            _chats.sort((c1, c2) => {
                const c1LastMessageIndex = c1.messages.length - 1; const c2LastMessageIndex = c2.messages.length - 1
                const c1Message = c1.messages[c1LastMessageIndex]; const c2Message = c2.messages[c2LastMessageIndex]
                const c1Timestamp = c1Message ? c1Message.timestamp : 0; const c2TimeStamp = c2Message ? c2Message.timestamp : 0
    ...
    Chat.findOne()
        ...
        .then(chat => {
            ...
            (chat.users.every(u => IChat.Role.isReader(u.role)) ? chat.remove() : chat.save())
                .then(
                    () => res.status(200).json({ description: "Chat has been deleted." }),
                    err => res.status(500).json({ description: err.message })
                )
        }, err => res.status(500).json({ description: err.message }))
}
...
export function updateChat(req, res) {
    ...
    Chat.findOne()
        ...
        .then(chat => {
            ...
            function checkAuthorization(user: DecodedTokenType) {
            ...
            function saveUpdatedChatUsers(chat: IChat, response: {res: any, message: string}) {
                chat.save()
                    .then(_chat => {
                        _chat.populate(ChatPopulationPipeline, (err, populatedChat) => {
                ...
                case IChat.Type.ONE:
                    {
                        switch (action as UpdateAction) {
                            ...
                            case UpdateAction.UPDATE_USER_ROLE: {
                                if (!body.role || !IChat.Role.values().includes(body.role))
                                    return res.status(400).json({ description: "Body must be of the form: { role: string } with 'role' in [" + IChat.Role.values() + "]" })
                                const _user = chat.users.find(u => u.user._id == user._id)
                                _user.role = body.role
                                chat.save()
                                    .then(
                                        () => res.status(200).json({ description: "Update my role in chat." }),
                                        err => res.status(500).json({ description: err.message })
                                    )
                            }
                                break
                        ...
                        switch (action as UpdateAction) {
                            case UpdateAction.UPDATE_CHAT_NAME: {
                                if (checkAuthorization(user)) {
                                    ...
                                    chat.save()
                                        .then(
                                            () => res.status(200).json({ description: "Update name of chat." }),
                                            err => res.status(500).json({ description: err.message })
                                        )
                                }
                            }
                                ...
                                areAllUserAdmin(chat, user)
                                    .then(allAdmin => {
                                        ...
                                        if (allAdmin) {
                                            ...
                                            chat.save()
                                                .then(
                                                    () => res.status(200).json({ description: "Update my role in chat with administrators." }),
                                                    err => res.status(500).json({ description: err.message })
                                                )
                                        } else {
                                            if (checkAuthorization(user)) {
                            ...
                            case UpdateAction.UPDATE_CHAT_IMAGE: {
                                if (checkAuthorization(user)) {
                                    chatMiddleware.uploadChatImage()(req, res, err => {
                                        ...
                                        chat.save()
                                            .then(
                                                () => res.status(200).json({ description: "Update image of chat." }),
                                                err => res.status(500).json({ description: err.message })
                                            )
                                    })
                                }
...
function areAllUserAdmin(chat: IChat, user: DecodedTokenType): Promise<boolean> {
    return User.find()
        ...
        .select("_id")
        .then(
            admins => Promise.resolve(chat.users.filter(u => u.user._id != user._id).every(u => _.findIndex(admins, { _id: u.user._id }) != -1)),
