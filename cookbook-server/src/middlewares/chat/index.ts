...
export function uploadChatImage(): Middleware {
    const config: UploaderConfiguration = {
        type: FileType.IMAGE,
...
function checkAndFormatBody(): Middleware {
    return async function (req, res, next) {
        ...
        if (body.message) _body.messages = body.message ? [new Message({ content: body.message, sender: id })] : undefined

        const me = _body.users.find(u => u.user === id)

        if (_body.users.length === 1 && me) {
            return next({ status: 400, description: "'Users' must contain at least one user different from " + id })
        }
        ...
        switch (_body.info.type as IChat.Type) {
            case IChat.Type.ONE:
                ...
                else _body.users.unshift({ user: id })
            ...
            case IChat.Type.GROUP:
                ...
                else _body.users.unshift({ user: id, role: IChat.Role.ADMIN })
                break
            default:
...
export function list(): Middlewares {
    return [
        ...
        function (req, res, next) {
            ...
            const noMessages = req.query["no-messages"]
            if (noMessages) {
                try {
            ...
            if (name) {
                const regexObject = { $regex: `^${name.value}`, $options: "i" }
