...
export default function (app) {
    ...
    app.get(routes.signupAdmins, (req, res) => res.status(200).render("signup-admin", { appName }))

    /**
     * SHOW REST API DOCUMENTATIONS
     */
    const swaggerCookbookAPI = YAML.load(path.resolve("api-docs", "api-documentations.yaml"))
    app.use(routes.apiDocs, (req, res, next) => {
        ...
        swaggerCookbookAPI
            ...
            .schema
            .example = configuration.server.origin
     ...
     * MAIN ROUTE
     */
    app.use((req, res) => res.status(200).render("index", { appName, routes }))
}
