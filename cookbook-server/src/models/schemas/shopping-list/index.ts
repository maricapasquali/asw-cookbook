...
ShoppingListSchema.pre("save", function () {
    const _foods: Set<string> = new Set(this.shoppingList.map(p => p.food.toString()))
    if (_foods.size < this.shoppingList.length) throw new MongooseDuplicateError("duplicate food on shopping list")
})
