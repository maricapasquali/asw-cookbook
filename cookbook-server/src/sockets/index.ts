    ...
    io.use(middlewareCheckAuthorization)
        .on("connection", socket => {
            socket.use(([event, ...args], next) => {
                const checkIfAuthenticationIsValid = /chat:.*|friendship:.*|recipe:(?!comment$).*|user:update:password|user:strike|food:create|check:.*/.test(event)
