...
export default function (io: any, socket: any): void {
        ...
        User.findOne()
            ...
            .then(_user => {
                if (_user) {
                    ...
                    socket.to(Rooms.ADMINS)
                        .emit("user:signup", user)
                    if (SignUp.State.isChecked(_user.signup)) socket.broadcast.except(Rooms.ADMINS)
                        .emit("user:checked", user)
    ...
    // RECIPE PERMISSION
    socket.on("recipe:add:permission", recipe => io.to(recipe?.permission?.map(p => p.user._id))
        .emit("recipe:add:permission", { recipe }))
    ...
    socket.on("shopping-list:add", point => {
        ...
        socket.to(id)
            .emit("shopping-list:add", point)
    })
    socket.on("shopping-list:update", point => {
        const { id } = userInformation(socket)
        socket.to(id)
            .emit("shopping-list:update", point)
    })
    socket.on("shopping-list:remove", pointID => {
        const { id } = userInformation(socket)
        socket.to(id)
            .emit("shopping-list:remove", pointID)
    })
}
