...
export function recipe(io: any, recipe: any, like: ILike): void {
    ...
    if (like.user) {
        createNotification({
            ...
            .then(notification => io.to(like.user._id)
                .emit("like:recipe", { notification, like }), err => console.error(err))
    }
    ...
    createNotification({
        ...
        .then(notification => io.to(recipeOwner)
            .emit("like:recipe", { notification, like }), err => console.error(err))

    io.local.except([recipeOwner, like.user?._id])
        .emit("like:recipe", { notification: { otherInfo }, like })
}
...
export function comment(io: any, comment: any, like: any): void {
    ...
    if (like.user) {
        createNotification({
            ...
            .then(notification => io.to(like.user._id)
                .emit("like:comment", { notification, like }), err => console.error(err))
    }
    ...
    if (comment.user) {
        createNotification({
            ...
            .then(notification => io.to(commentOwner)
                .emit("like:comment", { notification, like }), err => console.error(err))
    }

    io.local.except([commentOwner, like.user?._id])
        .emit("like:comment", { notification: { otherInfo }, like })
}
