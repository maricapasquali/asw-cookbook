...
export function request(io: any, request: IFriend): void {
    ...
    createNotification({
        ...
        .then(notification => io.to(request.from._id)
            .emit("friendship:request", notification), err => console.error(err))
    ...
    createNotification({
        ...
        .then(notification => io.to(request.to._id)
            .emit("friendship:request", notification), err => console.error(err))
}
...
export function update(io: any, request: IFriend): void {
    ...
    createNotification({
        ...
        .then(notification => io.to(request.to._id)
            .emit("friendship:update", { notification, friendship }), err => console.error(err))
    ...
    if (request.state == FriendShip.State.ACCEPTED) io.local.except([request.from._id, request.to._id])
        .emit("friendship:update", { friendship })
}

export function remove(io: any, user: UserInformationType, otherUser: { _id: string, userID: string }): void {
    const friendship = { from: { _id: user.id }, to: { _id: otherUser._id } }
    ...
    createNotification({
        ...
        .then(notification => io.to(user.id)
            .emit("friendship:remove", { notification, friendship }), err => console.error(err))
    ...
    createNotification({
        ...
        .then(notification => io.to(otherUser._id)
            .emit("friendship:remove", { notification, friendship }), err => console.error(err))

    io.local.except([user.id, otherUser._id])
        .emit("friendship:remove", { friendship })
}
