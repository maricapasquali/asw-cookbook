      ...
      <div v-if="!processing">
        <b-card v-if="link.valid">
          <div class="text-center">
            <logo />
            <h3 v-if="changeDefaultPassword">
              <em>Cambia DEFAULT password</em>
            </h3>
          </div>
          <b-card-body>
            <b-alert
              ...
              <b-container>
                ...
                >
                  ...
                  <b-col class="pr-0 text-right">
                    <b-button
                      v-if="success.userID && validation.password"
                      type="button"
                      variant="primary"
                      @click="change"
                    >
                ...
                >
                  ...
                  <b-col class="pr-0 text-right">
                    <b-button
                      v-if="validation.userID && !success.userID && !changeDefaultPassword"
                      type="button"
                      variant="primary"
                      @click="checkUser"
                    >
            ...
            <b-row cols="1">
              ...
              <b-col class="text-right">
                <router-link
                  ...
                  replace
                >
                  <b-button>Vai al Login</b-button>
                </router-link>
              </b-col>
            </b-row>
          </b-container>
...
export default {
    ...
    methods: {
        ...
        change() {
            ...
            this.$store.dispatch("users/reset-password/make", { userID:  this.temporaryUserIdentifier || this.id, newPassword: this.hash_password, accessToken: this.temporaryToken || this.accessToken })
                .then(res => {
                    console.debug("response ", res)
                    if (res?.status === 200) {
                        this.$router.replace(this.changeDefaultPassword ? { name:"p-user-account", params: { id: this.id } } : { name: "login" })
                    } else {
