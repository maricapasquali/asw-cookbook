<template>
  <b-skeleton-wrapper :loading="processing">
    ...
    <b-container
      v-if="doc"
      :class="areThereComments ? 'col-lg-11' : ''"
    >
      <b-row
        cols="1"
        :cols-lg="areThereComments? 2: 1"
        class="mx-0"
      >
        <b-col>
          <b-container fluid>
            <b-row cols="1">
              ...
              <b-col class="header-recipe my-4 px-0">
                ...
                >
                  ...
                  >
                    <b-col
                      v-if="doc.category || doc.diet"
                      class="px-0"
                    >
                      <b-row cols="1">
                        <b-col v-if="doc.category">
                          {{ nameCategory(doc.category) }}
                        </b-col>
                        <b-col v-if="doc.diet">
                          {{ nameDiet(doc.diet) }}
                        </b-col>
          ...
          <b-container fluid>
            ...
            >
              <preview-recipe-tutorial
                ...
                class="recipe-tutorial col col-10 mb-3"
                :poster="doc.img"
              />
...
export default {
    ...
    created() {
        ...
        window.isDefined
        if (!isDefined(this.value)) this.getRecipe()
    ...
    methods: {
        ...
        isOwner(ownerRecipe) {
            return ownerRecipe._id === this.userIdentifier && ownerRecipe.userID === this.username
        },
        setRecipe(data) {
            this.doc = data
        ...
        getRecipe() {
            ...
            let { id, recipeId } = this.$route.params
            this.processing = true
            this.$store.dispatch("recipes/one-shared", { ownerID: id, recipeID: recipeId, options })
        ...
        onDeletedUserListeners(id) {
            if (this.doc && this.doc.owner && this.doc.owner._id === id) {
                this.doc.owner = null
                this.itemsBreadcrumb = []
                if (this.$route.name !== "recipe") this.$router.replace({ name: "recipe", params: { recipeId: this.doc._id } })
            }
        },
