<template>
  <b-button-group size="sm">
    <b-button
      :variant="variant"
      :disabled="noLike"
      :title="titleActionLike"
      @click="onLike"
    >
      <b-icon-heart-fill
        v-if="makeLike || noLike"
        :class="classesLike"
      />
      <b-icon-heart
        v-else
    ...
    <b-dropdown
      ...
      :title="titleToggleListLiker"
      :variant="variant"
      @show="showListLikers=true"
      @hide="showListLikers=false"
    >
      <b-dropdown-item
        v-for="(liker, ind) in mappedLike"
        :key="ind"
        :disabled="liker.anonymous>0"
      >
        <avatar
          v-if="liker.user"
          v-model="liker.user.img"
          :size="30"
          :user="liker.user._id"
          :user-id="liker.user.userID"
          link
        />
        <span
          v-else-if="liker.anonymous"
          class="anonymous"
        > Anonimo <span v-if="liker.anonymous > 1">( x {{ liker.anonymous }} )</span></span>
      </b-dropdown-item>
    </b-dropdown>
  </b-button-group>
</template>
...
export default {
    ...
    computed: {
        ...
        titleActionLike() {
            return !this.noLike ? (this.makeLike ? "Non mi piace più" : "Mi Piace") : ""
        },
