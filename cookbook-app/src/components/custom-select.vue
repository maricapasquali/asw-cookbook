<template>
    <div class="col-12">
      <b-row>
        <b-col cols="12" :class="{'px-0': true, 'hidden': !useBasedSelect}">
          <b-container>
            <b-row align-v="center">
              <b-input-group-prepend class="col-0" v-if="isPassedSlotPrependIcon">
                <b-input-group-text class="py-2 rounded-left"> <slot name="icon-prepend"></slot>  </b-input-group-text>
              </b-input-group-prepend>
              <b-form-select :id="id" :value="selected.value" @input="select($event)" :class="customSelectClasses" :disabled="disabled">
                <b-form-select-option v-for="opt in $data._options" :key="opt.value" :value="opt.value" :disabled="opt.disabled"> {{opt.text}} </b-form-select-option>
              </b-form-select>
            </b-row>
          </b-container>
        </b-col>
        <!-- CUSTOM SELECT -->
        <b-col cols="12" class="px-0" v-if="!useBasedSelect">
          <b-container>
            <b-row align-v="center">
              <b-col cols="0" v-if="isPassedSlotPrependIcon">
                  <b-input-group-prepend>
                    <b-input-group-text class="py-2 rounded-left"> <slot name="icon-prepend"></slot>  </b-input-group-text>
                  </b-input-group-prepend>
              </b-col>

              <div :class="customSelectClasses"
                   :tabindex="tabindex"
                   :id="id"
                   :ref="customSelectId"
              ...
              >
                <b-row class="selected-input">
                  <b-col v-if="selected.value && selected.src" cols="1">
...
export default {
  ...
  methods:{
    ...
    _nextOpt(){
      ...
      if(this.indexSelected < this.$data._options.length - 1){
        this.selected = this.$data._options[++this.indexSelected]
    ...
    _disableScrollWindow(){
      if(!this.$data._disabledScrolling) {
        window.scrolling.disable()
        this.$data._disabledScrolling = true
      }
    },
    _enableScrollWindow(){
      ...
      if(this.$data._disabledScrolling) {
        ...
        this.$data._disabledScrolling = false
      }
    },
    ...
    setValueText(val){
      this.indexSelected = this.$data._options.findIndex(v => v.value === val)
      this.selected = this.indexSelected === -1 ? this.defaultValue : this.$data._options[this.indexSelected]
