<template>
  ...
  >
        ...
        >
          <b-input-group>
            ...
            <b-form-input
              ...
              v-model="searchChat"
              type="search"
              placeholder="Ricerca tra le chat avviate"
            />
          </b-input-group>
        </b-form-group>
      </b-col>
    ...
    >
      <b-col
        v-if="!processing && started.length===0"
        cols="12"
        class="pl-0"
      >
      ...
      >
        ...
        >
          <b-navbar-toggle
            v-if="started.length"
            ref="btn-chats-navigator"
            target="chat-items-navigator"
          >
            <template #default="{ expanded }">
              <b-icon
                ...
                icon="chevron-bar-up"
                variant="danger"
                title="Nascondi chat avviate"
              />
              <b-icon
                v-else
                icon="chevron-bar-down"
                variant="primary"
                title="Mostra chat avviate"
              />
            </template>
          </b-navbar-toggle>

          <b-collapse
            id="chat-items-navigator"
            is-nav
          >
            <b-navbar-nav class="ml-auto">
              <b-list-group class="chat-list">
                ...
                <b-list-group-item
                  v-for="startedChat in started"
                  :key="startedChat._id"
                  button
                  :active="_iSelectedChat(startedChat)"
                  @click="_clickItem(startedChat)"
                >
                  <chat-item
                    :ref="chatItemRef(startedChat._id)"
                    class="nav-item"
                    :value="startedChat"
                    @remove-chat="openDeleteModal"
                    @redirectOtherTab="redirectOtherTab"
                  />
                </b-list-group-item>
              </b-list-group>
            </b-navbar-nav>
      ...
      <b-col
        v-show="showChatContainer"
        class="chat-container px-0 py-0"
        cols="12"
        sm="12"
        md="12"
        lg="8"
    ...
    >
      <b-container>
            ...
            >
              <b-input-group>
                ...
                <b-form-input
                  ...
                  type="search"
                  :placeholder="'Ricerca ' + (isAdmin ? 'utente': 'amico')"
                />
              </b-input-group>
            </b-form-group>
          </b-col>
        ...
        >
          <b-col
            v-for="friend in _friends"
            :key="friend._id"
            class="friend-item px-0"
          >
            <chat-friend-item
              :value="friend"
              @clickUser="onAddNewChat"
            />
          </b-col>
    ...
    <!-- DELETE CHAT MODEL -->
    <b-modal
      v-model="deleteChat.show"
      centered
      title="Cancellazione chat"
      @ok="onRemoveChat"
    >
...
export default {
            ...
            handler(val) {
                ...
                if (!this.isSearchModeFriend && !this.isAdmin && this.friends.length === 0 && !this.justRequestFriend) {
                    ...
                    this.$store.dispatch("friendships/own", { state: "accepted", options })
                        .then(() => console.debug("retrieve own accepted friends."))
    ...
    methods: {
        ...
        onListenersPushMessage(chatInfo) {
