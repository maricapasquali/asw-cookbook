  ...
  >
    ...
    >
      <b-col>
        <b-row
          cols="1"
          class="chat h-100 px-0"
        >
          <b-col
            ref="chat-header"
            class="messages-header text-center px-0"
          >
            <b-col>
              <chat-header
                :value="value"
                @on-user-enter="enterChat"
                @on-user-leave="leaveChat({leaveUser: $event})"
              />
            </b-col>
            <b-col
              v-if="writeUsers.length"
              ref="col-typing"
              class="messages-typing pb-1"
            >
              <chat-typing
                :users="writeUsers"
                :group="isChatGroup"
          ...
          <b-col class="messages-container">
            ...
            >
              <b-row
                v-for="mex in messages"
                :key="mex._id"
                :class="_classSingleMessage(mex)"
                cols="1"
              >
                <b-col
                  cols="12"
                ...
                >
                  <chat-message
                    :value="mex"
                    :group="isChatGroup"
                    :attachment-api="getAttachmentsInfo"
            ...
            <div
              v-if="!amInReading && newMessages.length"
            ...
            >
              ...
              <b-button
                pill
                variant="secondary"
                :title="`${newMessages.length} nuovi messaggi`"
                @click="_clickToBottom"
              >
          ...
          <b-col
            v-if="!value || amINotReaderUser"
            ref="chat-footer"
            class="messages-footer"
          >
            <chat-footer
              :disabled="!value"
              :attachments-items="recipes"
              :attachment="attachment.link"
              :attachment-preview="attachment.preview"
              attachment-search-field="name"
              @send-text="sendMessage"
              @typing="sendTyping"
              @attachment-click="createLinkToSend"
              @write="onWriteMessage"
            >
              <template #attachment-item="{ item }">
                <b-card
                  no-body
                  img-left
                    ...
                    <b-col>
                      <b-card-body :title="item.name">
                        <b-row align-v="center">
                          ...
                          <b-col class="pl-1">
                            <span>{{ _recipeCategoryName(item.category) }}</span>
                          </b-col>
                        </b-row>
                      </b-card-body>
                    </b-col>
...
export default {
    ...
    computed: {
        ...
        temporaryNameChat: {
            ...
            set(val) {
                if (this.value) {
                    this.$emit("input", Object.assign(this.value, { info: Object.assign(this.value.info, { tempName: val }) }))
                }
        ...
        mappedAttachments() {
            return this.recipes.map(recipe => {
                let link = this._linkAttachment(recipe._id)
                return {
                    id: recipe._id,
                    link,
                    preview: this.createObjectPreview(recipe, link)
                }
            })
        }
    },
        ...
        messages: {
            ...
            handler(val, old) {
                ...
                if (val?.length === 1) {
                    ...
                    this.$emit("input", Object.assign(this.value, { started: true }))
                    this.$emit("start")
                }
    ...
    methods: {
        ...
        sendMessage(message, attachment) {
            ...
            this.whenAttachmentPresetUpdatePermission()
                ...
                .catch(e => {
                    console.error("NO UPDATE PERMISSION ", e); this._changeStateMyMessage({ delivered: false })
                })
        },
        ...
        onWriteMessage(val) {
            ...
            this.attachment = this.mappedAttachments.find(_attachment => val.indexOf(_attachment.link) !== -1) || { id: "", link: "", preview: {} }
            console.debug("this.attachment ", this.attachment)
        },
