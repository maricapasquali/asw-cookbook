...
export default {
    of({rootState}, {userID, state, pagination, options}){
       return rootState._api.friends.getFriendOf(userID, rootState.session.accessToken, {state}, pagination, options)
           .then(response =>{
    ...
    own({ commit, dispatch, rootState, rootGetters}, payload){
        ...
        return rootState._api.friends.getFriendOf(rootState.session.user._id, rootState.session.accessToken,{ userID, state }, pagination, options)
            .then(response =>{
    ...
    ['request-to']({ commit, dispatch, rootState, rootGetters}, userID){
        let isLoggedIn = rootGetters['session/isLoggedIn']
        if(!isLoggedIn) return dispatch('sayNotLoggedIn', null, { root: true })
        return rootState._api.friends.requestFriendShip(userID, rootState.session.accessToken)
            .then(response => {
    ...
    ['break-up-with']({commit, dispatch, rootState, rootGetters}, userID){
        let isLoggedIn = rootGetters['session/isLoggedIn']
        if(!isLoggedIn) return dispatch('sayNotLoggedIn', null, { root: true })
        return rootState._api.friends.breakFriendShip(rootState.session.user._id, userID, rootState.session.accessToken)
            .then(response => {
    ...
    ['update-request']({commit, dispatch, rootState, rootGetters}, {userID, state}){
        let isLoggedIn = rootGetters['session/isLoggedIn']
        if(!isLoggedIn) return dispatch('sayNotLoggedIn', null, { root: true })
        return rootState._api.friends.updateFriendShip(rootState.session.user._id, userID,{ state }, rootState.session.accessToken)
            .then(response => {
